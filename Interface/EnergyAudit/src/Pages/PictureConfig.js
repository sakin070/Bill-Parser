...
class PictureConfig extends React.Component {
  ...
  storeSelection = () => {
    ...
    this.setState(oldState => ({
      selections: oldState.selections.concat({ inputValue, currentSelection })
    }));
  ...
  saveConfig = () => {
    ...
    for (i = 0; i < this.state.selections.length; i++) {
      selections[this.state.selections[i]["inputValue"]] = this.rectToTupple(
        this.state.selections[i]["currentSelection"]
      );
    }
    ...
    data.append(
      ...
      JSON.stringify({ selections }).replace(
        "selections",
        this.state.configurationIdentifier
      )
    );
    ...
    //Set boolean to google token auth attempt
    const loggedIn = xhr.send("idtoken=" + localStorage.getItem("id_token"));
      ...
      return (
        <div className="ui text container">
          <h2 className="ui header">
            Create a new set of configuration selections
          </h2>
          <p>
            {" "}
            You only need to create a configuration if you have a lot of files
            to parse and the files share the same structure but different
            content. Otherwise you should try our direct parse
          </p>
          ...
          <p>
            ...
            <ul>
              <li>
                First you have to upload an image to base the configuration off
                of
              </li>
              <li>
                For each section you will like to include in the configuration
              </li>
              <ul>
              ...
              <li>
                {" "}
                Input the name you would like the configuration to be stored
                under
              </li>
              <li>Click the save configuration button</li>
              <li>Start by selecting the the file to template</li>
              <label htmlFor="files" className="ui primary button">
                Select File
              </label>
              <input
                id="files"
          ...
          <div className="col-lg-3" style={{ marginLeft: "10px" }}>
            <label>
              ...
              <input
                ...
                name="configID"
                onChange={this.updateConfigurationIdentifierValue}
              />
            </label>
            {/*<h4>Selections</h4>*/}
                  ...
                  <div className="row" style={{ marginBottom: "5px" }}>
                    ...
                    <div className="col">
                      ...
                      <input
                        ...
                        value={JSON.stringify(d.currentSelection, null, 2)}
                      />
                    </div>
                  </div>
                </form>
